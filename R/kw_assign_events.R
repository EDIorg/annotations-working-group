...
output_path <- "assigned_kw/events"
...
exclude_terms <- unlist(df_exclude_terms$exclude_events)
...
write.csv(df_datasets, file = file.path(input_path, "datasetKeywords_cleaned_events.csv"))

search_term <- readxl::read_excel(file.path(search_term_path,'search_term_mapping_events.xlsx'))

# Make an empty dataframe to write stuff into
df_ds_subset <- data.frame(packageid = character(0),
                           title = character(0),
                           events_level_1 = character(0),
                           events_level_2 = character(0),
                           events_level_3 = character(0))
...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(events_level_1 = search_term[[9]][j]) %>%
    mutate(events_level_2 = search_term[[10]][j]) %>%
    mutate(events_level_3 = search_term[[11]][j])
...
df_ds_sub_distinct <- df_ds_subset %>%
  distinct(packageid, events_level_1, events_level_2, events_level_3)
...
# df_ds_out <- df_ds_out %>%
#   mutate(del = if_else(str_detect(packageid, 'and|sgs|nwt') & events_level_2 == 'agricultural', 'rem', '')) %>%
#   mutate(del = if_else(str_detect(packageid, 'and|arc') & events_level_1 == 'coastal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'arc') & events_level_2 == 'marine', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes') & events_level_1 == 'polar', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes|bnz') & events_level_2 == 'island', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'hfr') & events_level_2 == 'intertidal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'sbc') & events_level_1 == 'terrestrial' & events_level_2 == 'forest', 'rem', del))
# 
# df_ds_out <- df_ds_out %>%
#   filter(del == '')
# -------------------------------------------
...
# Export output
write.csv(df_ds_out, file = 'assigned_kw/combined/events.csv', row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_events.csv', sep = ''), row.names = F)
}

# no events information

# Identify cases where the events is present
...
# Export the set of files that lack events information
# write.csv(df_eco_abs, file = file.path(output_path, 'no_events.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_no_events.csv', sep = ''), row.names = F)
}

# ----------------------------------
# write all files to google drive

upload_url <- googledrive::as_id("https://drive.google.com/drive/folders/1OngdxZVIbEtLpcBJHqgM-0eyqzeLmZhx")
