...
output_path <- "assigned_kw/organism"
...
exclude_terms <- unlist(df_exclude_terms$exclude_organism)
...
write.csv(df_datasets, file = file.path(input_path, "datasetKeywords_cleaned_organism.csv"))

search_term <- readxl::read_excel(file.path(search_term_path,'search_term_mapping_organism.xlsx'))

# Make an empty dataframe to write stuff into
df_ds_subset <- data.frame(packageid = character(0),
                           title = character(0),
                           organism_level_1 = character(0),
                           organism_level_2 = character(0),
                           organism_level_3 = character(0))
...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(organism_level_1 = search_term[[9]][j]) %>%
    mutate(organism_level_2 = search_term[[10]][j]) %>%
    mutate(organism_level_3 = search_term[[11]][j])
...
df_ds_sub_distinct <- df_ds_subset %>%
  distinct(packageid, organism_level_1, organism_level_2, organism_level_3)
...
# df_ds_out <- df_ds_out %>%
#   mutate(del = if_else(str_detect(packageid, 'and|sgs|nwt') & organism_level_2 == 'agricultural', 'rem', '')) %>%
#   mutate(del = if_else(str_detect(packageid, 'and|arc') & organism_level_1 == 'coastal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'arc') & organism_level_2 == 'marine', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes') & organism_level_1 == 'polar', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes|bnz') & organism_level_2 == 'island', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'hfr') & organism_level_2 == 'intertidal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'sbc') & organism_level_1 == 'terrestrial' & organism_level_2 == 'forest', 'rem', del))
# 
# df_ds_out <- df_ds_out %>%
#   filter(del == '')
# -------------------------------------------
...
# Export output
write.csv(df_ds_out, file = 'assigned_kw/combined/organism.csv', row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_organism.csv', sep = ''), row.names = F)
}

# no organism information

# Identify cases where the organism is present
...
df_pop <- df_eco_abs %>%
  filter(str_detect(text_combined, 'population'))

write.csv(df_pop, file = 'assigned_kw/combined/pop_no_organism.csv', row.names = F)

df_scope <- distinct(df_eco_abs, scope)

# Export the set of files that lack organism information
# write.csv(df_eco_abs, file = file.path(output_path, 'no_organism.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_no_organism.csv', sep = ''), row.names = F)
}

# ----------------------------------
# write all files to google drive

upload_url <- googledrive::as_id("https://drive.google.com/drive/folders/1C1WT8SZ8spwHvNkUeKGCzPZTAnrg2mU7")
