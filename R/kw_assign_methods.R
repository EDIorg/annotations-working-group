...
output_path <- "assigned_kw/methods"
...
exclude_terms <- unlist(df_exclude_terms$exclude_methods)
...
write.csv(df_datasets, file = file.path(input_path, "datasetKeywords_cleaned_methods.csv"))

search_term <- readxl::read_excel(file.path(search_term_path,'search_term_mapping_methods.xlsx'))

# Make an empty dataframe to write stuff into
df_ds_subset <- data.frame(packageid = character(0),
                           title = character(0),
                           methods_level_1 = character(0),
                           methods_level_2 = character(0),
                           methods_level_3 = character(0))
...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(methods_level_1 = search_term[[9]][j]) %>%
    mutate(methods_level_2 = search_term[[10]][j]) %>%
    mutate(methods_level_3 = search_term[[11]][j])
...
df_ds_sub_distinct <- df_ds_subset %>%
  distinct(packageid, methods_level_1, methods_level_2, methods_level_3)
...
# df_ds_out <- df_ds_out %>%
#   mutate(del = if_else(str_detect(packageid, 'and|sgs|nwt') & methods_level_2 == 'agricultural', 'rem', '')) %>%
#   mutate(del = if_else(str_detect(packageid, 'and|arc') & methods_level_1 == 'coastal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'arc') & methods_level_2 == 'marine', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes') & methods_level_1 == 'polar', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes|bnz') & methods_level_2 == 'island', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'hfr') & methods_level_2 == 'intertidal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'sbc') & methods_level_1 == 'terrestrial' & methods_level_2 == 'forest', 'rem', del))
# 
# df_ds_out <- df_ds_out %>%
#   filter(del == '')
# -------------------------------------------
...
# Export output
write.csv(df_ds_out, file = 'assigned_kw/combined/methods.csv', row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_methods.csv', sep = ''), row.names = F)
}

# no methods information

# Identify cases where the methods is present
...
# Export the set of files that lack methods information
# write.csv(df_eco_abs, file = file.path(output_path, 'no_methods.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_no_methods.csv', sep = ''), row.names = F)
}

# ----------------------------------
# write all files to google drive

upload_url <- googledrive::as_id("https://drive.google.com/drive/folders/1lQglEnwLLt9_iqtBigiP_ihQ_l8AxsJ1")

file_names <- dir(output_path)

for (j in 13:length(file_names)) {
