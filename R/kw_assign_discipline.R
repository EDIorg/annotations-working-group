...
output_path <- "assigned_kw/discipline"
...
exclude_terms <- unlist(df_exclude_terms$exclude_discipline)
...
write.csv(df_datasets, file = file.path(input_path, "datasetKeywords_cleaned_disciplines.csv"))

search_term <- readxl::read_excel(file.path(search_term_path,'search_term_mapping_discipline.xlsx'))

# Make an empty dataframe to write stuff into
df_ds_subset <- data.frame(packageid = character(0),
                           title = character(0),
                           discipline_level_1 = character(0),
                           discipline_level_2 = character(0),
                           discipline_level_3 = character(0))
...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(discipline_level_1 = search_term[[8]][j]) %>%
    mutate(discipline_level_2 = search_term[[9]][j]) %>%
    mutate(discipline_level_3 = search_term[[10]][j])
...
df_ds_sub_distinct <- df_ds_subset %>%
  distinct(packageid, discipline_level_1, discipline_level_2, discipline_level_3)
...
# df_ds_out <- df_ds_out %>%
#   mutate(del = if_else(str_detect(packageid, 'and|sgs|nwt') & discipline_level_2 == 'agricultural', 'rem', '')) %>%
#   mutate(del = if_else(str_detect(packageid, 'and|arc') & discipline_level_1 == 'coastal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'arc') & discipline_level_2 == 'marine', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes') & discipline_level_1 == 'polar', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes|bnz') & discipline_level_2 == 'island', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'hfr') & discipline_level_2 == 'intertidal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'sbc') & discipline_level_1 == 'terrestrial' & discipline_level_2 == 'forest', 'rem', del))
# 
# df_ds_out <- df_ds_out %>%
#   filter(del == '')
# -------------------------------------------
...
# Export output
# write.csv(df_ds_out, file = file.path(output_path,'discipline.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_discipline.csv', sep = ''), row.names = F)
}

# no discipline information

# Identify cases where the discipline is present
...
# Export the set of files that lack discipline information
# write.csv(df_eco_abs, file = file.path(output_path, 'no_discipline.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_no_discipline.csv', sep = ''), row.names = F)
}

# ----------------------------------
# write all files to google drive

upload_url <- googledrive::as_id("https://drive.google.com/drive/folders/1zlnP0SHlDGCvme9Kick23Wx6g1CRz7lJ")
