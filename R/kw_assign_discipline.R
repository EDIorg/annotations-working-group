...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(discipline_level_1 = search_term[[9]][j]) %>%
    mutate(discipline_level_2 = search_term[[10]][j]) %>%
    mutate(discipline_level_3 = search_term[[11]][j])
...
# Export output
write.csv(df_ds_out, file = 'assigned_kw/combined/discipline.csv', row.names = F)
