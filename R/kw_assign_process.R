...
output_path <- "assigned_kw/process"
...
exclude_terms <- unlist(df_exclude_terms$exclude_process)
...
write.csv(df_datasets, file = file.path(input_path, "datasetKeywords_cleaned_process.csv"))

search_term <- readxl::read_excel(file.path(search_term_path,'search_term_mapping_process.xlsx'))

# Make an empty dataframe to write stuff into
df_ds_subset <- data.frame(packageid = character(0),
                           title = character(0),
                           process_level_1 = character(0),
                           process_level_2 = character(0),
                           process_level_3 = character(0))
...
for (j in 1:nrow(search_term)) {
  ...
  df_ds_subset_row <- df_datasets %>%
    ...
    mutate(process_level_1 = search_term[[9]][j]) %>%
    mutate(process_level_2 = search_term[[10]][j]) %>%
    mutate(process_level_3 = search_term[[11]][j])
...
df_ds_sub_distinct <- df_ds_subset %>%
  distinct(packageid, process_level_1, process_level_2, process_level_3)
...
# df_ds_out <- df_ds_out %>%
#   mutate(del = if_else(str_detect(packageid, 'and|sgs|nwt') & process_level_2 == 'agricultural', 'rem', '')) %>%
#   mutate(del = if_else(str_detect(packageid, 'and|arc') & process_level_1 == 'coastal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'arc') & process_level_2 == 'marine', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes') & process_level_1 == 'polar', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'bes|bnz') & process_level_2 == 'island', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'hfr') & process_level_2 == 'intertidal', 'rem', del)) %>%
#   mutate(del = if_else(str_detect(packageid, 'sbc') & process_level_1 == 'terrestrial' & process_level_2 == 'forest', 'rem', del))
# 
# df_ds_out <- df_ds_out %>%
#   filter(del == '')
# -------------------------------------------
...
# Export output
write.csv(df_ds_out, file = 'assigned_kw/combined/process.csv', row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_process.csv', sep = ''), row.names = F)
}

# no process information

# Identify cases where the process is present
...
# Export the set of files that lack process information
# write.csv(df_eco_abs, file = file.path(output_path, 'no_process.csv'), row.names = F)
...
for (j in 1:nrow(df_scope)) {
  ...
  write.csv(df_ds_site, file = paste(output_path, '/', df_scope[[1]][j], '_no_process.csv', sep = ''), row.names = F)
}

# ----------------------------------
# write all files to google drive

upload_url <- googledrive::as_id("https://drive.google.com/drive/folders/1axGs4_aOzCPpZ8FNCnLJBOYkhRBaG7pI")
